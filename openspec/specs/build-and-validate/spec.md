...
Folio SHALL render validated pages into SVG files in a spec-local output directory by default.

#### Scenario: Default output directory
- **WHEN** a user runs `folio build` against a spec file or project directory without `--out-dir`
- **THEN** Folio writes rendered SVG pages to `<spec_dir>/out/`
...
- **THEN** Folio writes the rendered pages to that directory instead

#### Scenario: Single-page build
- **WHEN** a user provides `--page <n>`
- **THEN** Folio writes only the matching page
