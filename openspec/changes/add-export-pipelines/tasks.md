...
- [ ] 1.3 Preserve `pdf()` as the legacy SVG-rendered raster fallback and support `pdf(source="1080p")` without renaming the `pdf` target.
- [ ] 1.4 Add tests for unknown sources, unsupported routes, cycle diagnostics, and source compatibility rules.
...
- [ ] 2.3 Define artifact kinds for page SVG, page PNG, document PDF, document IDML, and document tree inputs.
- [ ] 2.4 Implement deterministic topological ordering and duplicate dependency coalescing.
- [ ] 2.5 Add focused planner tests for shared dependencies, transitive dependencies, and ordering.
...
- [ ] 3.4 Add a PDF page input abstraction that can represent PNG-backed pages now and future SVG-to-vector-PDF inputs later.
- [ ] 3.5 Keep IDML as a document-scoped built-in step that consumes the document tree.
- [ ] 3.6 Ensure dependency artifacts are reused within one build and only public outputs are written to `out/`.
