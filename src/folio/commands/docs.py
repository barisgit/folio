"""`folio docs` command group.

Exposes the committed DSL doc index to humans and agents. Subcommands:

- `folio docs show <symbol>` — detailed view of one symbol.
- `folio docs search <query>` — case-insensitive search across names,
  summaries, parameter names, and tags.
- `folio docs list [--kind=...]` — enumerate symbols, optionally filtered.
- `folio docs generate` — regenerate the committed `index.json` (dev alias
  for `python -m folio.docs.generate`).

Every subcommand accepts `--format={text,json,md}` (default `text`) and the
`--json` shortcut for `--format=json`. Mixing `--json` with a non-`json`
format exits with code 1.
"""
